...
export const Dashboard = ({ apiKey, socket }: { apiKey: string; socket: string; }) => {
  ...
  const applyStats = useCallback((stats: NodeStats) => {
    ...
    Promise.all(
      stats.dhtNodes.map(async (host) => ({ country: await getCountry(host.split(":")[0]!), host }))
  ...
  const SI = ({ k }: { k: keyof ApiPeer }): JSX.Element =>
    sortK !== k ? <span style={{ opacity: 0.2 }}>⇅</span>
    : sortD === 1 ? <span style={{ color: ACCENT }}>↑</span>
  ...
  return <div style={{ background: BG, color: TEXT, fontFamily: "'JetBrains Mono','Courier New',monospace", fontSize: 13, minHeight: "100vh" }}>
    ...
    {/* Nav */}
    <div style={{ alignItems: "center", background: "#010409", borderBottom: `1px solid ${BORD}`, display: "flex", height: 46, justifyContent: "space-between", padding: "0 16px", position: "sticky", top: 0, zIndex: 100 }}>
      <div style={{ alignItems: "center", display: "flex", gap: 0 }}>
        ...
        {(["overview", "peers", "dht", "votes", "logs", "search"] as const).map((t) => (
          <button className={`tab${tab === t ? " on" : ""}`} key={t} onClick={() => setTab(t)}>{t.toUpperCase()}</button>
        ))}
      </div>
    ...
    <div style={{ animation: "fadein .3s ease", padding: "14px 16px" }}>
      ...
      {tab === "peers" && <PeersTab filter={filter} sel={sel} setFilter={setFilter} setSel={setSel} sorted={sorted} />}
